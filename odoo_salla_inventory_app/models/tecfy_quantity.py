...
class TecfySallaQuantity(models.Model):
    ...
    def prepareQuantities(self):
        ...
        for company in companies:
            ...
            sql = "select p.id from product_product p left join product_template t on t.id = p.product_tmpl_id where t.detailed_type = 'product' and p.id not in (select product_id from tecfysalla_quantity where sync_id = {} ) and p.active = true limit {};".format(company.tecfy_salla_sync_id, count)
